...
`,
	...
	5. 上传目录并忽略 .svn 目录
	BaiduPCS-Go upload C:/Users/Administrator/Desktop /视频 --ignore .svn

	6. 上传目录并忽略多个目录
	BaiduPCS-Go upload C:/Users/Administrator/Desktop /视频 --ignore .svn --ignore .git
`,
			Category: "百度网盘",
			Before:   reloadFn,
			Action: func(c *cli.Context) error {
				...
				pcscommand.RunUpload(subArgs[:c.NArg()-1], subArgs[c.NArg()-1], &pcscommand.UploadOptions{
					...
					IgnoreDirs:    c.StringSlice("ignore"),
			...
			Flags: []cli.Flag{
				...
				cli.StringSliceFlag{
					Name:  "ignore",
					Usage: "上传时忽略的目录名，可多次指定，例如: --ignore .svn --ignore .git",
				},
				cli.BoolFlag{
					Name:  "no-history",
					Usage: "禁用上传历史记录，每次上传都重新检查服务器",
				},
			},
		},
		{
