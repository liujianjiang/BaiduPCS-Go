...
func WalkDir(dirPth, suffix string) (files []string, err error) {
	return WalkDirWithIgnore(dirPth, suffix, nil)
}

// WalkDirWithIgnore 获取指定目录及所有子目录下的所有文件，可以匹配后缀过滤，并忽略指定的目录。
// ignoreDirs 是要忽略的目录名列表，例如 []string{".svn", ".git"}
// 支持 Linux/macOS 软链接
func WalkDirWithIgnore(dirPth, suffix string, ignoreDirs []string) (files []string, err error) {
	...
	for _, dir := range ignoreDirs {
		ignoreMap[strings.ToUpper(dir)] = true
	}
	...
	walkFunc = func(filename string, fi fs.DirEntry, err error) error { //遍历目录
		...
		if fi.IsDir() {
			// 检查是否在忽略目录列表中
			if len(ignoreMap) > 0 && ignoreMap[strings.ToUpper(fi.Name())] {
