...
func RunUpload(localPaths []string, savePath string, opt *UploadOptions) {
	...
	// 打开上传历史记录数据库
	var uploadHistory *pcsupload.UploadHistory
	if !opt.NoHistory {
		uploadHistory, err = pcsupload.NewUploadHistory()
		if err != nil {
			fmt.Printf("警告: 打开上传历史数据库失败: %s, 将不使用历史记录\n", err)
		} else {
			defer uploadHistory.Close()
			// 清理无效记录
			uploadHistory.Clear()
		}
	}
	...
	for k := range localPaths {
		walkedFiles, err := pcsutil.WalkDirWithIgnore(localPaths[k], "", opt.IgnoreDirs)
		if err != nil {
